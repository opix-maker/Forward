...
async function main() {
    console.log('Starting intelligent build process v2...');
    ...
    try {
        ...
        // 为每个榜单配置的抓取任务获取ID
        for (const config of Object.values(BUILD_MATRIX)) {
            if (config.crawlParams) {
        ...
        for (const [key, config] of Object.entries(BUILD_MATRIX)) {
            if (config.type === 'series') {
                ...
                for (const [seriesId, seriesData] of seriesCollection.entries()) {
                    if (seriesData.items.length < 2) continue; // 至少有2部才算系列
