...
const KEYWORD_TO_THEME_MAP = {
    'theme:cyberpunk': ['cyberpunk', 'dystopia', 'virtual reality', 'artificial intelligence', 'neo-noir'],
    'theme:space-opera': ['space opera', 'alien', 'galaxy', 'spaceship'],
    'theme:time-travel': ['time travel', 'time loop', 'paradox'],
    'theme:post-apocalyptic': ['post-apocalyptic', 'dystopian'],
    'theme:superhero': ['superhero', 'marvel comics', 'dc comics', 'comic book'],
    'theme:mecha': ['mecha', 'giant robot'],
    'theme:zombie': ['zombie', 'undead'],
    'theme:monster': ['vampire', 'werewolf', 'monster', 'kaiju'], 
    'theme:ghost': ['ghost', 'haunting', 'supernatural horror'],
    'theme:slasher': ['slasher', 'body horror', 'folk horror'],
    'theme:magic': ['magic', 'sword and sorcery'],
    'theme:gangster': ['gangster', 'mafia', 'mobster', 'yakuza'], 
    'theme:heist': ['heist'],
    'theme:film-noir': ['film-noir', 'hardboiled'],
    'theme:spy': ['conspiracy', 'spy', 'espionage', 'assassin'],
    'theme:serial-killer': ['serial killer'],
    'theme:whodunit': ['whodunit'],
    'theme:courtroom': ['courtroom drama'],
    'theme:wuxia': ['wuxia', 'martial arts', 'kung fu'],
    'theme:xianxia': ['xianxia'],
    'theme:samurai': ['samurai', 'ninja'],
    'theme:isekai': ['isekai'],
    'theme:slice-of-life': ['slice of life', 'high school'],
    'theme:found-footage': ['found footage'],
};
...
const GATED_THEMES = {
    ...
    'theme:zombie': ['恐怖', '科幻'],
    'theme:monster': ['恐怖', '科幻', '奇幻', '动作'],
    'theme:slasher': ['恐怖', '惊悚'],
    'theme:serial-killer': ['犯罪', '惊悚', '恐怖', '悬疑'],
};

// --- 预编译正则表达式 ---
const COMPILED_THEME_REGEX = Object.entries(KEYWORD_TO_THEME_MAP).flatMap(([theme, keywords]) => 
...
const GENRE_MAP_EN_TO_CN = {
    ...
    'Romance': '爱情', 'Sci-Fi': '科幻', 'Science Fiction': '科幻', 
    'Sport': '运动', 'Thriller': '惊悚', 'War': '战争', 'Western': '西部',
    'Short': '短片', 'News': '新闻', 'Reality-TV': '真人秀', 'Talk-Show': '脱口秀',
    'Game-Show': '游戏节目',
};
// 创建一个反向映射，用于将中文也标准化
const GENRE_MAP_CN_TO_CN = {
    '动作': '动作', '冒险': '冒险', '动画': '动画', '传记': '传记', '喜剧': '喜剧',
    '犯罪': '犯罪', '纪录片': '纪录片', '剧情': '剧情', '家庭': '家庭', '奇幻': '奇幻',
    '黑色电影': '黑色电影', '历史': '历史', '恐怖': '恐怖', '音乐': '音乐', '歌舞': '歌舞',
    '悬疑': '悬疑', '爱情': '爱情', '科幻': '科幻', '运动': '运动', '惊悚': '惊悚',
    '战争': '战争', '西部': '西部', '短片': '短片', '新闻': '新闻', '真人秀': '真人秀',
    '脱口秀': '脱口秀', '游戏节目': '游戏节目',
};
...
function normalizeGenre(rawGenre) {
    ...
    // 直接匹配
    let normalized = GENRE_NORMALIZER[rawGenre];
    if (normalized) return normalized;

    // 处理大小写和空格
    const trimmed = rawGenre.trim();
    const capitalized = trimmed.charAt(0).toUpperCase() + trimmed.slice(1).toLowerCase();
    normalized = GENRE_NORMALIZER[capitalized] || GENRE_NORMALIZER[trimmed];
...
export function analyzeAndTagItem(item, imdbAkasInfo = { regions: new Set(), languages: new Set() }, imdbGenresString = null) {
    ...
    if (imdbGenresString && imdbGenresString !== '\\N') {
        imdbGenresString.split(',').forEach(g => rawGenres.add(g.trim()));
    }
    // 标准化并打标签
    ...
    rawGenres.forEach(raw => {
        ...
        if (normalized) {
            standardizedGenres.add(normalized);
        }
    });
    ...
    imdbAkasInfo.regions.forEach(r => allCountries.add(r)); // 来自IMDb AKAS
    
    imdbAkasInfo.languages.forEach(l => allLanguages.add(l)); // 来自IMDb AKAS
    ...
    tmdbKeywords.forEach(tmdbKeyword => {
        for (const { regex, theme } of COMPILED_THEME_REGEX) {
    ...
    foundThemes.forEach(theme => {
        ...
        if (requiredGenres) {
            if (requiredGenres.some(g => standardizedGenres.has(g))) {
                tags.add(theme);
            }
        } else {
            tags.add(theme); // 无门控要求，直接添加
        }
    });
